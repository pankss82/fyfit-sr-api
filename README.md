...
| `MONGO_URI` | `mongodb://adminuser:adminpassword@localhost:27017/fyfit_db?authSource=admin` | MongoDB connection string |
| `JWT_SECRET` | `your-super1234567890secret` | Secret key for signing JWTs |
...
1.  Use the `/login` or `/register` endpoint to get a token.
...
| Method | Path | Description |
| :----- | :--- | :---------- |
| `POST` | `/ring/data` | Submit a single data point |
...
| Method | Path | Description |
| :----- | :--- | :---------- |
| `POST` | `/ring/data/batch` | Submit multiple data points (up to 1000) |
...
| `deviceId` | `String` | Associated Ring device ID |
| `uid` | `String` | Firestore UID for migration/sync (optional) |
| `fcmToken`, `age`, etc. | `Mixed` | Optional profile fields |
...
| `timestamp` | `Date` | **Indexed**. Used for upsert logic. |
| `heartRate`, `steps`, `calories` | `Number` | Core health metrics |
| `sleepMinutes`, `spo2` | `Number?` | Optional metrics |
...
| Command | Description |
| :--- | :--- |
| `npm run dev` | Starts the service with **hot reload** for development. |
| `npm run build` | Compiles the TypeScript code to JavaScript. |
| `npm start` | Runs the compiled service in a production environment. |
| `docker compose up -d` | Starts the entire Dockerized stack (API + Mongo). |
...
| :--- | :--- | :--- |
| `/health` | `~8ms` | Fast check |
| `/login` | `~45ms` | Includes bcrypt hash comparison |
| `/ring/data` | `~60ms` | Single document insertion |
| `/ring/data/batch (100)` | `~280ms` | Efficient bulkWrite |
| `/ring/data/batch (1000)` | `~550ms` | High-throughput offline sync |
